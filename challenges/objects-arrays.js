...
graduates.forEach((uni) => {
  let uniFinal = `${uni.university}`;
  universities.push(uniFinal);
  universities.sort();
});
...
graduates.forEach((nameEmail) => {
  let finalArr = `${nameEmail.first_name} ${nameEmail.email}`; 
  contactInfo.push(finalArr);
})
...
//const uni = graduates.filter(uniNameString)
...
zooAnimals.forEach((habitatName) => {
  let nameAndScientific = `Name: ${habitatName.animal_name}, Scientific: ${habitatName.scientific_name}`;
  animalNames.push(nameAndScientific);
});
...
zooAnimals.map((lowerNames) => {
  let finalName = `${lowerNames.animal_name.toLowerCase()}`;
  lowerCase.push(finalName);
});
...
*/
const largerPopulation = zooAnimals.filter((animalPop) => {
  return `${animalPop.population}` < 5;
});
...
*/
const populationTotal = zooAnimals.reduce((totalPop, zoo) => {
  return totalPop += zoo.population;
}, 0);
